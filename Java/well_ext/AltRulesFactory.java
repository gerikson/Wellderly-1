...
public class AltRulesFactory extends ScalarFunctionFactory {
	@Override
	...
	public void getReturnType(ServerInterface srvInterface,
		...
		outputTypes.addVarchar(inputTypes.getColumnType(2).getStringLength(),
				"Alt");
	}
	...
	public class AltRules extends ScalarFunction {
			...
			do {
				...
				if (type.equals("del")) {
					...
					for (int i = 0; i < alt.length(); i++) {
						...
						no_change = true;
						for (int j = mod_alt.length(); j > 0; j--) {
							if (no_change) {
